...
Boot_StatusTypeDef ComTransmit(const uint8_t *data, uint32_t length, uint32_t timeout_ms);
Boot_StatusTypeDef ComReceive(uint8_t *data, uint32_t length, uint32_t timeout_ms);
